# The External Lobe — TDD Reimplementation Plan

> A dependency-ordered, test-first plan to rebuild the system described in
> [`SPECIFICATION.md`](./SPECIFICATION.md). The ordering follows the testability
> gradient: **pure logic → stores → hooks → components → integration**. Each
> milestone is shippable and gated by a green test suite.
...
- Tests encode §4.1 exactly: weight-profile sums, boundary scores (0 and 1),
...
- Tests: Ollama happy path; JSON extraction from markdown-wrapped responses;
  timeout via `AbortController`; retry with exponential backoff (assert delays via
  fake timers); Ollama→cloud fallback; **cloud path scrubs PII first**;
...
- Tests: LLM path validates + type-guards before returning; **fallback to
  ...
  interest/duration/title per spec; `generateMicroActions` length 3–7 + template
  fallback; `executePipeline` produces a valid `Task`, logs latency, flags review
  when needed; `quickCapture` is LLM-free and fast.
...
- `LightningCapture` (auto-focus, friction log > 5 s warning, save with timestamp),
  `CaptureModal` (Cmd-K), `CaptureCommandLine` (prefix parsing), `CaptureGestureCanvas`.
- `AirlockGatekeeper`: time-gated render (inject "now"); hidden nav until first capture.
...
- `EnergySlider` (1–5, re-rank on change), `ActivationBridge` (UI lock, sequential
  gating, momentum unlock → DEEP_FOCUS), `DetailScentsHUD` (blurred-by-default,
  click-to-reveal), `EngineStatus`, `BodyDoubleNudge` (subscribes to nudge counter),
...
- Second-brain variations (Split/Notebook/Terminal) — test the terminal query
  parser as pure logic first.
...
- Playwright covering the 8 **Global Acceptance Scenarios** (§13 of the spec):
